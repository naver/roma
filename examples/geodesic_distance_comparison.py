...
for label, input_angles in [
    (
        "zero",
        np.concatenate((np.linspace(-0.005, 0.0, 1000, endpoint=True), np.linspace(0.0, 0.005, 1000, endpoint=True))),
    ),
    (
        "pi",
        np.concatenate(
            (
                np.linspace(np.pi - 0.005, np.pi, 1000, endpoint=True),
                np.linspace(np.pi, np.pi + 0.005, 1000, endpoint=True),
            )
        ),
    ),
]:
    ...
    fig.patch.set_alpha(0.0)  # no background
    ax.patch.set_alpha(0.0)  # no background
    ...
    if nan_count > 0:
        plt.scatter(
            np.asarray(input_angles)[mask],
            0.5 * np.ones(nan_count),
            marker="x",
            s=1.0,
            c="C02",
            label="rotmat_geodesic_distance_naive NaN",
        )
    ...
    if nan_count > 0:
        plt.scatter(
            ...
            s=1.0,
            c="C03",
            label="rotmat_geodesic_distance NaN",
        )
