...
class TestEuler(unittest.TestCase):
    ...
    def test_euler_unitquat_consistency(self):
        ...
        for degrees in (True, False):
            for batch_shape in [tuple(), torch.Size((30,)), torch.Size((50, 60))]:
                for intrinsics in (True, False):
                    for convention in ["".join(permutation) for permutation in itertools.permutations("xyz")] + [
                        "xyx",
                        "xzx",
                        "yxy",
                        "yzy",
                        ...
                        if degrees:
                            ...
                            self.assertTrue(
                                all([torch.all(angle > -np.pi) and torch.all(angle <= np.pi) for angle in angles])
                            )
